...
std::string HttpClient::build_request(
  ...
  if ((url.is_https() && url.get_port() != 443) ||
    ...
    if (!profile.sec_ch_ua.empty() && config_.send_browser_headers) {
...
HttpResponse HttpClient::request(
    ...
    if (config_.proxy_url.empty()) {
        ...
        if (parsed_url.is_https()) {
            ...
            if (!profile.tls_cipher_suites.empty()) {
                ...
                SSL_CTX_set_cipher_list(ssl_ctx->native_handle(), 
                                       cipher_string.str().c_str());
            }
            
            // Set supported curves to match browser
            if (!profile.tls_curves.empty()) {
                ...
                SSL_CTX_set1_groups_list(ssl_ctx->native_handle(),
                                        curves_string.str().c_str());
            }
    ...
    if (config_.follow_redirects && 
        ...
        if (config_.redirect_count >= config_.max_redirects) {
            throw std::runtime_error("Too many redirects (max: " + 
        ...
        if (location_it != response.headers.end()) {
            ...
            return redirect_client.request(redirect_method, redirect_url, 
                                          headers, redirect_body);
        }
    }
